  ...
  <div>
    ...
    >  
     <el-row>
      ...
      <el-col :span="12">
        <el-form-item   v-if="flag=='dianyuan'"  label="工号" prop="gonghao">
          <el-input v-model="ruleForm.gonghao" readonly              placeholder="工号" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='dianyuan'"  label="员工姓名" prop="yuangongxingming">
          <el-input v-model="ruleForm.yuangongxingming"               placeholder="员工姓名" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item v-if="flag=='dianyuan'"  label="性别" prop="xingbie">
          <el-select v-model="ruleForm.xingbie"  placeholder="请选择性别">
            <el-option
                v-for="(item,index) in dianyuanxingbieOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item   v-if="flag=='dianyuan'"  label="邮箱" prop="youxiang">
          <el-input v-model="ruleForm.youxiang"               placeholder="邮箱" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='dianyuan'"  label="联系手机" prop="lianxishouji">
          <el-input v-model="ruleForm.lianxishouji"               placeholder="联系手机" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="24">  
        <el-form-item v-if="flag=='dianyuan'" label="相片" prop="xiangpian">
          <file-upload
          tip="点击上传相片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.xiangpian?ruleForm.xiangpian:''"
          @change="dianyuanxiangpianUploadChange"
          ></file-upload>
        </el-form-item>
      </el-col>
...
export default {
  ...
  mounted() {
    ...
    this.dianyuanxingbieOptions = "男,女".split(',')
  },
  methods: {
    yonghuxiangpianUploadChange(fileUrls) {
        this.ruleForm.xiangpian = fileUrls;
    },
    dianyuanxiangpianUploadChange(fileUrls) {
        this.ruleForm.xiangpian = fileUrls;
    },
    onUpdateHandler() {
      ...
      if((!this.ruleForm.gonghao)&& 'dianyuan'==this.flag){
        this.$message.error('工号不能为空');
        return
      }
      if((!this.ruleForm.mima)&& 'dianyuan'==this.flag){
        this.$message.error('密码不能为空');
        return
      }
      if((!this.ruleForm.yuangongxingming)&& 'dianyuan'==this.flag){
        this.$message.error('员工姓名不能为空');
        return
      }
      if( 'dianyuan' ==this.flag && this.ruleForm.youxiang&&(!isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮箱格式`);
        return
      }
      if( 'dianyuan' ==this.flag && this.ruleForm.lianxishouji&&(!isMobile(this.ruleForm.lianxishouji))){
