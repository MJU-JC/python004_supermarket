  ...
  <div>
      ...
      <div class="login-form">
		...
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm">
			...
			<!-- <div v-if="tableName=='dianyuan'" class="input-group">
			   <div class="label">工号</div>
			   <div class="input-container">
			     <input v-model="ruleForm.gonghao" class="input" type="text" placeholder="工号">
			   </div>
			 </div> -->
			<el-form-item label="工号" class="input" v-if="tableName=='dianyuan'">
			...
			<el-form-item label="密码" class="input" v-if="tableName=='dianyuan'">
			  <el-input v-model="ruleForm.mima" autocomplete="off" placeholder="密码" type="password"#elsetype="text" />
			</el-form-item>
			<el-form-item label="确认密码" class="input" v-if="tableName=='dianyuan'">
			  <el-input v-model="ruleForm.mima2" autocomplete="off" placeholder="确认密码" type="password"/>
			</el-form-item>
			...
			<el-form-item label="员工姓名" class="input" v-if="tableName=='dianyuan'">
			...
			<el-form-item label="联系手机" class="input" v-if="tableName=='dianyuan'">
...
export default {
  ...
  methods: {
	...
	var url=this.tableName+"/register";
      ...
      if((!this.ruleForm.gonghao) && `dianyuan` == this.tableName){
        this.$message.error(`工号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `dianyuan` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      if((this.ruleForm.mima!=this.ruleForm.mima2) && `dianyuan` == this.tableName){
	    this.$message.error(`两次密码输入不一致`);
	    return
      }
      if((!this.ruleForm.yuangongxingming) && `dianyuan` == this.tableName){
        this.$message.error(`员工姓名不能为空`);
        return
      }
      if(`dianyuan` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮件格式`);
        return
      }
      if(`dianyuan` == this.tableName && this.ruleForm.lianxishouji&&(!this.$validate.isMobile(this.ruleForm.lianxishouji))){
