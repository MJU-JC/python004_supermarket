...
def shangpinxinxi_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def shangpinxinxi_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select goodtype from orders where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'shangpinxinxi' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from shangpinxinxi where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from shangpinxinxi where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def shangpinxinxi_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM shangpinxinxi {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def shangpinxinxi_alipay(request):
    if request.method in ["POST", "GET"]:
        alipay = AliPay(
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangodh92s/shangpinxinxi/notify',
            #notify_url=''
        )
...
def shangpinxinxi_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        
        return redirect('http://localhost:8080/djangodh92s/admin/dist/index.html#/shangpinxinxi')
