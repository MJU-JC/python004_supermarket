...
def yonghu_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select goodtype from orders where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'yonghu' order by addtime desc")
...
def yonghu_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangodh92s/yonghu/notify',
            #notify_url=''
        )
...
def yonghu_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/djangodh92s/admin/dist/index.html#/yonghu')
