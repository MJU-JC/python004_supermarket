...
def shangpinleixing_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def shangpinleixing_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select goodtype from orders where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'shangpinleixing' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from shangpinleixing where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from shangpinleixing where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def shangpinleixing_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM shangpinleixing {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def shangpinleixing_alipay(request):
    if request.method in ["POST", "GET"]:
        alipay = AliPay(
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangodh92s/shangpinleixing/notify',
            #notify_url=''
        )
...
def shangpinleixing_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        
        return redirect('http://localhost:8080/djangodh92s/admin/dist/index.html#/shangpinleixing')
